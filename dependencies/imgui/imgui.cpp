...
// support, e.g: https://www.patreon.com/imgui
...
examples/opengl_example/ first as it is the simplest. you may be able to grab
and copy a ready made imgui_impl_*** file from the examples/.
...
ImGui::GetIO() and fill the fields marked 'Input' 2/ call ImGui::NewFrame() as
early as you can! 3/ use any ImGui function you want between NewFrame() and
Render() 4/ call ImGui::Render() as late as you can to end the frame and
finalize render data. it will call your RenderDrawListFn handler that you set in
the IO structure. (if you don't need to render, you still need to call Render()
and ignore the callback, or call EndFrame() instead. if you call neither some
...
font yet unsigned char* pixels; int width, height;
...
functions anytime ImGui::NewFrame();
...
from the IO structure to tell how ImGui intends to use your inputs and to know
if you should share them or hide them from the rest of your application. Read
...
function will convert an old TitleBg/TitleBgActive color into a new one with the
same visual output, given the OLD color and the OLD WindowBg color. ImVec4
...
ImGuiSetCond). Kept inline redirection function (will obsolete).
...
multiple fonts. no need for a PNG loader. (1.30) - removed GetDefaultFontData().
uses io.Fonts->GetTextureData*() API to retrieve uncompressed pixels. this
sequence: const void* png_data; unsigned int png_size;
...
breaking changes. If you have a problem with a function, search for its name in
the code, there will likely be a comment about it. Please report any issue to
the GitHub page!
...
identify textures. You can safely store an integer into a void*, just cast it to
void*, don't take it's address!) To display a custom image/texture within an
...
disappearing when I move windows around.. A: Most likely you are mishandling the
clipping rectangles in your render function. Rectangles provided by ImGui are
defined as (x1,y1,x2,y2) and NOT as (x1,y1,width,height).

 Q: Can I have multiple widgets with the same label? Can I have widget without a
label? (Yes) A: Yes. A primer on the use of labels/IDs in ImGui..
...
they need an unique ID. unique ID are typically derived from a string label, an
...
(different from above) Button("Play##foo2");   // Label = "Play",   ID = hash of
"Play##foo2" (different from above)
...
identified by their ID.. obviously) Use "###" to pass a label that isn't part of
ID:
...
each tree node. Depending on your use cases you may want to use strings, indices
or pointers as ID. e.g. when displaying a single object that may change over
time (1-1 relationship), using a static string as ID will preserve your node
open/closed state when the targeted object change. e.g. when displaying a list
of objects, using indices or pointers as ID will preserve the node open/closed
state differently. experiment and see what makes more sense!

 Q: How can I tell when ImGui wants my mouse/keyboard inputs and when I can pass
them to my application? A: You can read the 'io.WantCaptureXXX' flags in the
ImGuiIO structure. Preferably read them after calling ImGui::NewFrame() to avoid
those flags lagging by one frame, but either should be fine. When
'io.WantCaptureMouse' or 'io.WantCaptureKeyboard' flags are set you may want to
discard/hide the inputs from the rest of your application. When
'io.WantInputsCharacters' is set to may want to notify your OS to popup an
on-screen keyboard, if available. ImGui is tracking dragging and widget activity
that may occur outside the boundary of a window, so 'io.WantCaptureMouse' is a
more accurate and complete than testing for ImGui::IsMouseHoveringAnyWindow().
...
size_in_pixels); ImFont* font2 = io.Fonts->AddFontFromFileTTF("myfontfile2.ttf",
size_in_pixels); io.Fonts->GetTexDataAsRGBA32() or GetTexDataAsAlpha8()
...
ranges); io.Fonts->LoadFromFileTTF("myfontfile.ttf", size_pixels, NULL, &config,
...
the glyphs to load. All your strings needs to use UTF-8 encoding. Specifying
literal in your source code using a local code page (such as CP-923 for Japanese
or CP-1251 for Cyrillic) will not work. In C++11 you can encode a string literal
in UTF-8 by using the u8"hello" syntax. Otherwise you can convert yourself to
UTF-8 or load text data from file already saved as UTF-8. You can also try to
remap your local codepage characters to their Unicode codepoint using
font->AddRemapChar(), but international users may have problems reading/editing
your source code.
...
background alpha, then retrieve the ImDrawList* via GetWindowDrawList() and draw
...
https://github.com/ocornut/imgui/issues The list below consist mostly of notes
of things to do before they are requested/discussed by users (at that point it
usually happens on the github)
...
// ImGui::SetCurrentContext() ImGui is currently not thread-safe because of this
// variable. If you want thread-safety to allow N threads to access N different
// contexts, you might work around it by (A) having two instances of the ImGui
// code under different namespaces or (B) change this variable to be TLS.
// Further development aim to make this context pointer explicit to all calls.
...
ImGuiStyle::ImGuiStyle() {
  ...
  DisplayWindowPadding = ImVec2(
	  22, 22); // Window positions are clamped to be visible within the display
			   // area by at least this amount. Only covers regular windows.
  ...
  CurveTessellationTol =
	  1.25f; // Tessellation tolerance. Decrease for highly tessellated curves
...
#ifdef __APPLE__
  WordMovementUsesAltKey = true; // OS X style: Text editing cursor movement
								 // using Alt instead of Ctrl
  ShortcutsUseSuperKey =
	  true; // OS X style: Shortcuts using Cmd/Super instead of Ctrl
  DoubleClickSelectsWord = true;  // OS X style: Double click selects by word
								  // instead of selecting whole text
  MultiSelectUsesSuperKey = true; // OS X style: Multi-selection in lists uses
...
// ImGui::MemFree()
void *ImLoadFileToMemory(const char *filename, const char *file_open_mode,
...
LowerBound(ImVector<ImGuiStorage::Pair> &data, ImU32 key) {
...
void ImGuiStorage::SetBool(ImU32 key, bool val) { SetInt(key, val ? 1 : 0); }
...
void ImGuiTextBuffer::appendv(const char *fmt, va_list args) {
  ...
  int len = vsnprintf(NULL, 0, fmt,
					  args); // FIXME-OPT: could do a first pass write attempt,
...
bool ImGui::ItemAdd(const ImRect &bb, const ImGuiID *id) {
  ...
  if (IsMouseHoveringRect(bb.Min, bb.Max)) {
	...
	// ActiveId==window->MoveID (we clicked on the window background) So that
	// clicking on items with no active id such as Text() still returns true
	// with IsItemHovered()
...
void ImGui::SetCurrentContext(ImGuiContext *ctx) { GImGui = ctx; }
...
void ImGui::NewFrame() {
  ...
  // Check user data
  IM_ASSERT(g.IO.DeltaTime >=
			0.0f); // Need a positive DeltaTime (zero is tolerated but will
				   // cause some timing issues)
  IM_ASSERT(g.IO.DisplaySize.x >= 0.0f && g.IO.DisplaySize.y >= 0.0f);
  IM_ASSERT(g.IO.Fonts->Fonts.Size >
			0); // Font Atlas not created. Did you call
				// io.Fonts->GetTexDataAsRGBA32 / GetTexDataAsAlpha8 ?
  IM_ASSERT(
	  g.IO.Fonts->Fonts[0]
		  ->IsLoaded()); // Font Atlas not created. Did you call
						 // io.Fonts->GetTexDataAsRGBA32 / GetTexDataAsAlpha8 ?
  ...
  if ((g.IO.MousePos.x < 0 && g.IO.MousePos.y < 0) ||
	  (g.IO.MousePosPrev.x < 0 &&
	   g.IO.MousePosPrev.y <
		   0)) // if mouse just appeared or disappeared (negative coordinate) we
...
static void AddDrawListToRenderList(ImVector<ImDrawList *> &out_render_list,
  ...
  IM_ASSERT(draw_list->_IdxWritePtr ==
  ...
  // ImDrawIdx = 2 bytes = 64K vertices) If this assert triggers because you are
  // drawing lots of stuff manually, A) workaround by calling
  // BeginChild()/EndChild() to put your draw commands in multiple draw lists,
  // B) #define ImDrawIdx to a 'unsigned int' in imconfig.h and render
  // accordingly.
  IM_ASSERT(
	  (int64_t)draw_list->_VtxCurrentIdx <=
	  ((int64_t)1L
	   << (sizeof(ImDrawIdx) *
...
void ImGui::EndFrame() {
  ...
  IM_ASSERT(g.FrameCountEnded !=
			g.FrameCount); // ImGui::EndFrame() called multiple times, or forgot
						   // to call ImGui::NewFrame() again
...
void ImGui::Render() {
  ...
  if (g.Style.Alpha > 0.0f) {
	...
	if (g.IO.MouseDrawCursor) {
	  ...
	  g.OverlayDrawList.AddImage(tex_id, pos + ImVec2(1, 0),
								 pos + ImVec2(1, 0) + size,
								 cursor_data.TexUvMin[1],
								 cursor_data.TexUvMax[1], 0x30000000); // Shadow
	  g.OverlayDrawList.AddImage(tex_id, pos + ImVec2(2, 0),
								 pos + ImVec2(2, 0) + size,
								 cursor_data.TexUvMin[1],
								 cursor_data.TexUvMax[1], 0x30000000); // Shadow
	  g.OverlayDrawList.AddImage(
		  tex_id, pos, pos + size, cursor_data.TexUvMin[1],
		  cursor_data.TexUvMax[1], 0xFF000000); // Black border
	  g.OverlayDrawList.AddImage(
		  tex_id, pos, pos + size, cursor_data.TexUvMin[0],
		  cursor_data.TexUvMax[0], 0xFFFFFFFF); // White fill
...
void ImGui::RenderTextClipped(const ImVec2 &pos_min, const ImVec2 &pos_max,
							  ...
							  ImGuiAlign align, const ImVec2 *clip_min,
...
void ImGui::RenderCollapseTriangle(ImVec2 p_min, bool is_open, float scale,
  ...
  if (shadow && (window->Flags & ImGuiWindowFlags_ShowBorders) != 0)
	window->DrawList->AddTriangleFilled(a + ImVec2(2, 2), b + ImVec2(2, 2),
										c + ImVec2(2, 2),
...
static bool IsPopupOpen(ImGuiID id) {
  ImGuiContext &g = *GImGui;
  const bool is_open = g.OpenPopupStack.Size > g.CurrentPopupStack.Size &&
...
static bool BeginPopupEx(const char *str_id, ImGuiWindowFlags extra_flags) {
  ...
  if (flags & ImGuiWindowFlags_ChildMenu)
	ImFormatString(name, 20, "##menu_%d",
...
//    that the item isn't interactable (because it is blocked by the active
...
bool ImGui::BeginChild(ImGuiID id, const ImVec2 &size, bool border,
					   ImGuiWindowFlags extra_flags) {
  char str_id[32];
...
static ImVec2 FindBestPopupWindowPos(const ImVec2 &base_pos, const ImVec2 &size,
	   ...
	   n++) // Last, Right, down, up, left. (Favor last used direction).
  {
	...
	if (rect.GetWidth() < size.x || rect.GetHeight() < size.y)
	  continue;
	*last_dir = dir;
	return ImVec2(dir == 0   ? r_inner.Max.x
				  : dir == 3 ? r_inner.Min.x - size.x
							 : base_pos_clamped.x,
				  dir == 1   ? r_inner.Max.y
				  : dir == 2 ? r_inner.Min.y - size.y
							 : base_pos_clamped.y);
  }
...
bool ImGui::Begin(const char *name, bool *p_open,
  ...
  if (g.SetNextWindowPosCond) {
	const ImVec2 backup_cursor_pos =
		window->DC
			.CursorPos; // FIXME: not sure of the exact reason of this
						// saving/restore anymore :( need to look into that.
		...
		rect_to_avoid =
			ImRect(parent_window->Pos.x + style.ItemSpacing.x, -FLT_MAX,
				   parent_window->Pos.x + parent_window->Size.x -
					   style.ItemSpacing.x - parent_window->ScrollbarSizes.x,
	...
	if (window->Collapsed) {
	  ...
	  if (!(flags & ImGuiWindowFlags_NoTitleBar))
		window->DrawList->AddRectFilled(
			...
			GetColorU32(
				(g.FocusedWindow && window->RootNonPopupWindow ==
	  ...
	  if (flags & ImGuiWindowFlags_MenuBar) {
		...
		if (flags & ImGuiWindowFlags_ShowBorders)
		  window->DrawList->AddLine(menu_bar_rect.GetBL() - ImVec2(0, 0),
									menu_bar_rect.GetBR() - ImVec2(0, 0),
	...
	if (!(flags & ImGuiWindowFlags_NoTitleBar)) {
	  ...
	  if (!(flags & ImGuiWindowFlags_NoCollapse))
		RenderCollapseTriangle(window->Pos + style.FramePadding,
							   !window->Collapsed, 1.0f, true);

	  ImVec2 text_min = window->Pos + style.FramePadding;
	  ImVec2 text_max =
		  window->Pos + ImVec2(window->Size.x - style.FramePadding.x,
							   style.FramePadding.y * 2 + text_size.y);
	  ...
	  RenderTextClipped(text_min, text_max, name, NULL, &text_size,
						style.WindowTitleAlign, &clip_min, &clip_max);
	}
...
static void Scrollbar(ImGuiWindow *window, bool horizontal) {
  ...
  // scrollable amount) But we maintain a minimum size in pixel to allow for the
  // user to still aim inside.
...
void ImGui::PopFont() {
  ...
  SetCurrentFont(g.FontStack.empty() ? g.IO.Fonts->Fonts[0]
									 : g.FontStack.back());
}
...
static float *GetStyleVarFloatAddr(ImGuiStyleVar idx) {
  ...
  case ImGuiStyleVar_Alpha:
	return &g.Style.Alpha;
  case ImGuiStyleVar_WindowRounding:
	return &g.Style.WindowRounding;
  case ImGuiStyleVar_ChildWindowRounding:
	return &g.Style.ChildWindowRounding;
  case ImGuiStyleVar_FrameRounding:
	return &g.Style.FrameRounding;
  case ImGuiStyleVar_IndentSpacing:
	return &g.Style.IndentSpacing;
  case ImGuiStyleVar_GrabMinSize:
	return &g.Style.GrabMinSize;
  }
  return NULL;
}
...
void ImGui::PushStyleVar(ImGuiStyleVar idx, float val) {
  ...
  IM_ASSERT(pvar !=
			NULL); // Called function with wrong-type? Variable is not a float.
...
void ImGui::PushStyleVar(ImGuiStyleVar idx, const ImVec2 &val) {
  ...
  IM_ASSERT(pvar !=
			NULL); // Called function with wrong-type? Variable is not a ImVec2.
  ImGuiStyleMod backup;
  backup.Var = idx;
...
void ImGui::SetScrollY(float scroll_y) {
  ...
  window->ScrollTarget.y =
	  scroll_y + window->TitleBarHeight(); // title bar height canceled out when
										   // using ScrollTargetRelY
...
void ImGui::SetScrollFromPosY(float pos_y, float center_y_ratio) {
  ...
  if (center_y_ratio <= 0.0f &&
	  window->ScrollTarget.y <=
		  window->WindowPadding
			  .y) // Minor hack to make "scroll to top" take account of
				  // WindowPadding, else it would scroll to (WindowPadding.y -
				  // ItemSpacing.y)
...
void ImGui::LabelTextV(const char *label, const char *fmt, va_list args) {
  ...
  RenderTextClipped(value_bb.Min, value_bb.Max, value_text_begin,
					value_text_end, NULL, ImGuiAlign_VCenter);
...
bool ImGui::ButtonEx(const char *label, const ImVec2 &size_arg,
  ...
  if ((flags & ImGuiButtonFlags_AlignTextBaseLine) &&
	  style.FramePadding.y < window->DC.CurrentLineTextBaseOffset)
  ...
  RenderTextClipped(bb.Min, bb.Max, label, NULL, &label_size,
					ImGuiAlign_Center | ImGuiAlign_VCenter);
...
static inline void DataTypeFormatString(ImGuiDataType data_type, void *data_ptr,
  ...
  if (data_type == ImGuiDataType_Int) {
	...
	if (decimal_precision < 0)
	  ImFormatString(
		  buf, buf_size, "%f",
		  *(float *)data_ptr); // Ideally we'd have a minimum decimal precision
							   // of 1 to visually denote that it is a float,
							   // while hiding non-significant digits?
	else
...
bool ImGui::InputScalarAsWidgetReplacement(const ImRect &aabb,
  ...
  if (g.ScalarAsInputTextId == 0) {
	...
	IM_ASSERT(g.ActiveId ==
			  id); // InputText ID expected to match the Slider ID (else we'd
				   // need to store them both, which is also possible)
...
bool ImGui::SliderBehavior(const ImRect &frame_bb, ImGuiID id, float *v,
  ...
  if (decimal_precision > 0)
	...
	grab_sz = ImMin(
		ImMax(1.0f * (slider_sz / (v_max - v_min + 1.0f)), style.GrabMinSize),
  ...
  if (is_non_linear) {
	...
	if (v_clamped < 0.0f) {
	  ...
	  grab_t =
		  linear_zero_pos + powf(f, 1.0f / power) * (1.0f - linear_zero_pos);
	}
  } else {
	// Linear slider
...
bool ImGui::VSliderFloat(const char *label, const ImVec2 &size, float *v,
  ...
  RenderTextClipped(
	  ...
	  frame_bb.Max, value_buf, value_buf_end, NULL, ImGuiAlign_Center);
  if (label_size.x > 0.0f)
	RenderText(ImVec2(frame_bb.Max.x + style.ItemInnerSpacing.x,
					  frame_bb.Min.y + style.FramePadding.y),
			   label);
...
void ImGui::PlotEx(ImGuiPlotType plot_type, const char *label,
  ...
  ImVec2 tp0 = ImVec2(
	  t0, 1.0f - ImSaturate(
					 (v0 - scale_min) /
					 (scale_max - scale_min))); // Point in the normalized space
												// of our target rectangle

  const ImU32 col_base =
	  GetColorU32((plot_type == ImGuiPlotType_Lines) ? ImGuiCol_PlotLines
  ...
  if (overlay_text)
	RenderTextClipped(
		ImVec2(frame_bb.Min.x, frame_bb.Min.y + style.FramePadding.y),
		frame_bb.Max, overlay_text, NULL, NULL, ImGuiAlign_Center);
...
void ImGui::ProgressBar(float fraction, const ImVec2 &size_arg,
  ...
  if (overlay_size.x > 0.0f)
	RenderTextClipped(
		...
		ImGuiAlign_Left | ImGuiAlign_VCenter, &bb.Min, &bb.Max);
}
...
bool ImGui::InputTextEx(const char *label, char *buf, int buf_size,
  ...
  if (focus_requested || user_clicked || user_scrolled) {
	if (g.ActiveId != id) {
	  ...
	  if (!is_multiline &&
		  (focus_requested_by_tab || (user_clicked && is_ctrl_down)))
		select_all = true;
	}
	SetActiveID(id, window);
	FocusWindow(window);
  ...
  if (g.ActiveId == id) {
	...
	// Edit in progress
	const float mouse_x =
		(g.IO.MousePos.x - frame_bb.Min.x - style.FramePadding.x) +
		edit_state.ScrollX;
	const float mouse_y =
		(is_multiline ? (g.IO.MousePos.y - draw_window->DC.CursorPos.y -
						 style.FramePadding.y)
					  : (g.FontSize * 0.5f));

	if (select_all ||
		(hovered && !io.DoubleClickSelectsWord && io.MouseDoubleClicked[0])) {
	  edit_state.SelectAll();
	  edit_state.SelectedAllMouseLock = true;
	} else if (hovered && io.DoubleClickSelectsWord &&
	...
	if (g.IO.InputCharacters[0]) {
	  ...
	  if (!(is_ctrl_down && !is_alt_down) && is_editable) {
		for (int n = 0;
			 n < IM_ARRAYSIZE(g.IO.InputCharacters) && g.IO.InputCharacters[n];
			 n++)
	...
	const bool is_shortcutkey_only =
		(io.ShortcutsUseSuperKey ? (is_super_down && !is_alt_down &&
									!is_shift_down && !is_ctrl_down)
								 : (is_ctrl_down && !is_alt_down &&
									!is_shift_down && !is_super_down));
	const bool is_wordmove_key_down =
		(io.WordMovementUsesAltKey ? io.KeyAlt : io.KeyCtrl);

	if (IsKeyPressedMap(ImGuiKey_LeftArrow)) {
	  edit_state.OnKeyPressed(is_wordmove_key_down
								  ? STB_TEXTEDIT_K_WORDLEFT | k_mask
								  : STB_TEXTEDIT_K_LEFT | k_mask);
	} else if (IsKeyPressedMap(ImGuiKey_RightArrow)) {
	  edit_state.OnKeyPressed(is_wordmove_key_down
								  ? STB_TEXTEDIT_K_WORDRIGHT | k_mask
	  ...
	  edit_state.OnKeyPressed(is_ctrl_down ? STB_TEXTEDIT_K_TEXTSTART | k_mask
										   : STB_TEXTEDIT_K_LINESTART | k_mask);
	} else if (IsKeyPressedMap(ImGuiKey_End)) {
	  edit_state.OnKeyPressed(is_ctrl_down ? STB_TEXTEDIT_K_TEXTEND | k_mask
	  ...
	  if (is_ctrl_down && !edit_state.HasSelection())
	  ...
	  if (!is_multiline || (ctrl_enter_for_new_line && !is_ctrl_down) ||
		  (!ctrl_enter_for_new_line && is_ctrl_down)) {
		SetActiveID(0);
		enter_pressed = true;
	  } else if (is_editable) {
		unsigned int c = '\n'; // Insert new line
		if (InputTextFilterCharacter(&c, flags, callback, user_data))
		  edit_state.OnKeyPressed((int)c);
			   ...
			   IsKeyPressedMap(ImGuiKey_Tab) && !is_ctrl_down &&
			   !is_shift_down && !is_alt_down && is_editable) {
	  ...
	  SetActiveID(0);
	  cancel_edit = true;
	} else if (is_shortcutkey_only && IsKeyPressedMap(ImGuiKey_Z) &&
			   is_editable) {
	  edit_state.OnKeyPressed(STB_TEXTEDIT_K_UNDO);
	  edit_state.ClearSelection();
	} else if (is_shortcutkey_only && IsKeyPressedMap(ImGuiKey_Y) &&
			   is_editable) {
	  ...
	  edit_state.SelectAll();
	  edit_state.CursorFollow = true;
	} else if (is_shortcutkey_only && !is_password &&
	...
	} else if (is_shortcutkey_only && IsKeyPressedMap(ImGuiKey_V) &&
			   is_editable) {
	  // Paste
  ...
  if (is_multiline) {
	...
	EndGroup();
	if (g.ActiveId == id ||
		is_currently_scrolling) // Set LastItemId which was lost by
								// EndChild/EndGroup, so user can use
								// IsItemActive()
...
bool ImGui::InputFloat(const char *label, float *v, float step, float step_fast,
  ...
  if (decimal_precision < 0)
	strcpy(display_format,
		   "%f"); // Ideally we'd have a minimum decimal precision of 1 to
...
bool ImGui::Combo(const char *label, int *current_item, const char **items,
...
bool ImGui::ListBox(const char *label, int *current_item, const char **items,
...
bool ImGui::BeginMenu(const char *label, bool enabled) {
  ...
  if (menu_is_open) {
	...
	menu_is_open = BeginPopupEx(
		label, flags); // menu_is_open can be 'false' when the popup is
					   // completely clipped (e.g. zero size display)
  }
...
bool ImGui::ColorEdit4(const char *label, float col[4], bool alpha) {
  ...
  case ImGuiColorEditMode_HSV: {
	...
	const char **fmt = hide_prefix ? fmt_table[0]
					   : hsv       ? fmt_table[2]
								   : fmt_table[1];
...
void ImGui::Separator() {
  ...
  const ImRect bb(ImVec2(x1, window->DC.CursorPos.y),
				  ImVec2(x2, window->DC.CursorPos.y));
  ItemSize(ImVec2(
	  0.0f,
	  0.0f)); // NB: we don't provide our width so that it doesn't get feed back
			  // into AutoFit   // FIXME: Height should be 1.0f not 0.0f ?
  if (!ItemAdd(bb, NULL)) {
	if (window->DC.ColumnsCount > 1)
	  PushColumnClipRect();
	return;
...
void ImGui::SameLine(float pos_x, float spacing_w) {
  ...
  if (pos_x != 0.0f) {
	if (spacing_w < 0.0f)
	  spacing_w = 0.0f;
	window->DC.CursorPos.x =
		window->Pos.x - window->Scroll.x + pos_x + spacing_w;
...
void ImGui::NewLine() {
  ...
  if (window->SkipItems)
	return;
  if (window->DC.CurrentLineHeight >
...
static float GetDraggedColumnOffset(int column_index) {
  ...
  IM_ASSERT(
	  column_index >
	  0); // We cannot drag column 0. If you get this assert you may have a
  ...
  if (window->DC.ColumnsCount != 1) {
	...
	for (int column_index = 0; column_index < columns_count + 1;
	  ...
	  const float t = window->DC.StateStorage->GetFloat(
		  column_id,
		  default_t); // Cheaply store our floating point value inside the
					  // integer (could store an union into the map?)
	  window->DC.ColumnsData[column_index].OffsetNorm = t;
	}
  ...
  if (ImGui::Begin("ImGui Metrics", p_open)) {
	  ...
	  static void NodeDrawList(ImDrawList *draw_list, const char *label) {
		...
		for (const ImDrawCmd *pcmd = draw_list->CmdBuffer.begin();
		  ...
		  ImGuiListClipper clipper(
			  pcmd->ElemCount /
			  3); // Manually coarse clip our print out of individual vertices
	...
	if (ImGui::TreeNode("Basic state")) {
	  ...
	  ImGui::Text(
		  ...
		  g.HoveredIdPreviousFrame); // Data is "in-flight" so depending on when
