...
void ImDrawList::PathArcToFast(const ImVec2 &centre, float radius, int amin,
							   int amax) {
...
void ImDrawList::PathRect(const ImVec2 &a, const ImVec2 &b, float rounding,
  ...
  r = ImMin(r, fabsf(b.x - a.x) *
					   (((rounding_corners & (1 | 2)) == (1 | 2)) ||
								((rounding_corners & (4 | 8)) == (4 | 8))
							? 0.5f
							: 1.0f) -
				   1.0f);
  r = ImMin(r, fabsf(b.y - a.y) *
					   (((rounding_corners & (1 | 8)) == (1 | 8)) ||
...
void ImDrawList::AddRectFilledMultiColor(const ImVec2 &a, const ImVec2 &c,
										 ...
										 ImU32 col_bot_right,
										 ImU32 col_bot_left) {
  if (((col_upr_left | col_upr_right | col_bot_right | col_bot_left) >> 24) ==
	  0)
	return;
...
void ImDrawList::AddImage(ImTextureID user_texture_id, const ImVec2 &a,
						  const ImVec2 &b, const ImVec2 &uv0, const ImVec2 &uv1,
						  ImU32 col) {
  if ((col >> 24) == 0)
	return;
...
bool ImFontAtlas::Build() {
  ...
  for (int input_i = 0; input_i < ConfigData.Size; input_i++) {
	...
	IM_ASSERT(cfg.DstFont && (!cfg.DstFont->IsLoaded() ||
							  cfg.DstFont->ContainerAtlas == this));
  ...
  // if they wish. After packing is done, width shouldn't matter much, but some
  // API/GPU have texture size limitations and increasing width can decrease
  // height.
  TexWidth = (TexDesiredWidth > 0)        ? TexDesiredWidth
  ...
  stbrp_pack_rects((stbrp_context *)spc.pack_info, &extra_rects[0],
				   extra_rects.Size);
  ...
  stbtt_packedchar *buf_packedchars = (stbtt_packedchar *)ImGui::MemAlloc(
	  total_glyph_count * sizeof(stbtt_packedchar));
  stbrp_rect *buf_rects =
	  (stbrp_rect *)ImGui::MemAlloc(total_glyph_count * sizeof(stbrp_rect));
  stbtt_pack_range *buf_ranges = (stbtt_pack_range *)ImGui::MemAlloc(
	  total_glyph_range_count * sizeof(stbtt_pack_range));
  ...
  for (int input_i = 0; input_i < ConfigData.Size; input_i++) {
	...
	float off_y = (cfg.MergeMode && cfg.MergeGlyphCenterV)
					  ? (ascent - dst_font->Ascent) * 0.5f
					  : 0.0f;

	dst_font->FallbackGlyph =
		NULL; // Always clear fallback so FindGlyph can return NULL. It will be
			  // set again in BuildLookupTable()
	for (int i = 0; i < tmp.RangesCount; i++) {
	  ...
	  for (int char_idx = 0; char_idx < range.num_chars; char_idx += 1) {
		...
		stbtt_GetPackedQuad(range.chardata_for_range, TexWidth, TexHeight,
...
void ImFontAtlas::RenderCustomTexData(int pass, void *p_rects) {
  ...
  // blank) The white texels on the top left are the ones we'll use everywhere
  // in ImGui to render filled shapes.
...
const ImWchar *ImFontAtlas::GetGlyphRangesJapanese() {
  ...
  // unicode codepoint 0x4E00. Each offset has an implicit +1. This encoding
  // helps us reduce the source code size.
...
const char *ImFont::CalcWordWrapPositionA(float scale, const char *text,
  ...
  while (s < text_end) {
	...
	const float char_width = ((int)c < IndexXAdvance.Size)
								 ? IndexXAdvance[(int)c] * scale
								 : FallbackXAdvance;
	if (ImCharIsSpace(c)) {
	  if (inside_word) {
	...
	if (const Glyph *glyph = FindGlyph((unsigned short)c)) {
	  ...
	  if (c != ' ' && c != '\t') {
		...
		if (x1 <= clip_rect.z && x2 >= clip_rect.x) {
		  ...
		  // much overhead in a debug build. Inlined here:
		  {
