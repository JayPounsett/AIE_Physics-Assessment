...
Texture::Texture()
	: m_filename("none"), m_width(0), m_height(0), m_glHandle(0), m_format(0),
...
Texture::Texture(unsigned int width, unsigned int height, Format format,
	...
	: m_filename("none"), m_width(width), m_height(height), m_format(format),
	  m_loadedPixels(nullptr) {
...
bool Texture::load(const char *filename) {
  ...
  if (m_loadedPixels != nullptr) {
	...
	case STBI_grey:
	  m_format = RED;
	  glTexImage2D(GL_TEXTURE_2D, 0, GL_RED, x, y, 0, GL_RED, GL_UNSIGNED_BYTE,
				   m_loadedPixels);
	  break;
	case STBI_grey_alpha:
	  m_format = RG;
	  glTexImage2D(GL_TEXTURE_2D, 0, GL_RG, x, y, 0, GL_RG, GL_UNSIGNED_BYTE,
				   m_loadedPixels);
	  break;
	case STBI_rgb:
	  m_format = RGB;
	...
	case STBI_rgb_alpha:
	  ...
	  glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, x, y, 0, GL_RGBA,
				   GL_UNSIGNED_BYTE, m_loadedPixels);
	  break;
	default:
	  break;
	};
...
void Texture::create(unsigned int width, unsigned int height, Format format,
					 unsigned char *pixels) {
