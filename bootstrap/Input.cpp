...
Input::Input() {
  ...
  // set up callbacks
  auto KeyPressCallback = [](GLFWwindow *window, int key, int scancode,
							 int action, int mods) {
  ...
  auto MouseInputCallback = [](GLFWwindow *window, int button, int action,
							   int mods) {
	for (auto &f : Input::getInstance()->m_mouseButtonCallbacks)
	  f(window, button, action, mods);
  };

  auto MouseScrollCallback = [](GLFWwindow *window, double xoffset,
								double yoffset) {
...
const std::vector<int> &Input::getPressedKeys() const { return m_pressedKeys; }
...
double Input::getMouseScroll() { return m_mouseScroll; }
...
int Input::getMouseDeltaX() { return m_mouseX - m_oldMouseX; }

int Input::getMouseDeltaY() { return m_mouseY - m_oldMouseY; }
