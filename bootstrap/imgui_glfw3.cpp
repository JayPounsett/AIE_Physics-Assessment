...
void ImGui_MouseButtonCallback(GLFWwindow *, int button, int action,
							   int /*mods*/) {
...
bool ImGui_Init(GLFWwindow *window, bool install_callbacks) {
  ...
  io.RenderDrawListsFn =
	  ImGui_RenderDrawLists; // Alternatively you can set this to NULL and call
							 // ImGui::GetDrawData() after ImGui::Render() to
  ...
  if (glfwGetWindowAttrib(g_Window, GLFW_FOCUSED)) {
	...
	io.MousePos =
		ImVec2((float)mouse_x,
			   (float)mouse_y); // Mouse position in screen coordinates (set to
