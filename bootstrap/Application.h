...
public:
  ...
  // creates a window and begins the game loop which calls update() and draw()
  // repeatedly it first calls startup() and if that succeeds it then starts the
  // loop, ending with shutdown() if m_gameOver is true
  ...
  // sets m_gameOver to true which will close the application safely when the
  // frame ends
