...
Font::Font(const char *trueTypeFontFile, unsigned short fontHeight)
	: m_glyphData(nullptr), m_glHandle(0), m_pixelBufferHandle(0),
  ...
  if (file != nullptr) {
	...
	glBufferData(GL_PIXEL_UNPACK_BUFFER, m_textureWidth * m_textureHeight,
				 nullptr, GL_STREAM_COPY);
	unsigned char *tempBitmapData = (GLubyte *)glMapBufferRange(
		GL_PIXEL_UNPACK_BUFFER, 0, m_textureWidth * m_textureHeight,
		GL_MAP_WRITE_BIT | GL_MAP_INVALIDATE_BUFFER_BIT);

	m_glyphData = new stbtt_bakedchar[256];
	memset(m_glyphData, 0, sizeof(stbtt_bakedchar) * 256);
	stbtt_BakeFontBitmap(ttf_buffer, 0, fontHeight, tempBitmapData,
						 m_textureWidth, m_textureHeight, 0, 256,
	...
	glTexImage2D(GL_TEXTURE_2D, 0, GL_R8, m_textureWidth, m_textureHeight, 0,
...
float Font::getStringWidth(const char *str) {
  ...
  while (*str != 0) {
	stbtt_GetBakedQuad((stbtt_bakedchar *)m_glyphData, m_textureWidth,
					   m_textureHeight, (unsigned char)*str, &xPos, &yPos, &Q,
					   1);
...
void Font::getStringRectangle(const char *str, float &x0, float &y0, float &x1,
							  float &y1) {
