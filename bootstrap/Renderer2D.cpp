...
Renderer2D::Renderer2D() {
  ...
  glBufferData(GL_ELEMENT_ARRAY_BUFFER,
			   (MAX_SPRITES * 6) * sizeof(unsigned short),
			   (void *)(&m_indices[0]), GL_STATIC_DRAW);
  glBufferData(GL_ARRAY_BUFFER, (MAX_SPRITES * 4) * sizeof(SBVertex),
...
void Renderer2D::begin() {
  ...
  auto projection = glm::ortho(m_cameraX, m_cameraX + (float)width, m_cameraY,
							   m_cameraY + (float)height, 1.0f, -101.0f);
  glUniformMatrix4fv(glGetUniformLocation(m_shader, "projectionMatrix"), 1,
					 false, &projection[0][0]);
...
void Renderer2D::drawBox(float xPos, float yPos, float width, float height,
...
void Renderer2D::drawSprite(Texture *texture, float xPos, float yPos,
							float width, float height, float rotation,
							float depth, float xOrigin, float yOrigin) {
  if (texture == nullptr)
...
void Renderer2D::drawSpriteTransformed3x3(Texture *texture,
										  float *transformMat3x3, float width,
...
void Renderer2D::drawSpriteTransformed4x4(Texture *texture,
										  float *transformMat4x4, float width,
										  float height, float depth,
										  float xOrigin, float yOrigin) {
  if (texture == nullptr)
	texture = m_nullTexture;
...
void Renderer2D::drawText(Font *font, const char *text, float xPos, float yPos,
						  float depth) {
  ...
  while (*text != 0) {
	...
	stbtt_GetBakedQuad((stbtt_bakedchar *)font->m_glyphData,
					   font->m_textureWidth, font->m_textureHeight,
...
void Renderer2D::flushBatch() {
  ...
  glBufferSubData(GL_ARRAY_BUFFER, 0, m_currentVertex * sizeof(SBVertex),
				  m_vertices);
  glBufferSubData(GL_ELEMENT_ARRAY_BUFFER, 0,
				  m_currentIndex * sizeof(unsigned short), m_indices);
...
unsigned int Renderer2D::pushTexture(Texture *texture) {
  ...
  // if we've used all the textures we can, than we need to flush to make room
  // for another texture change
...
void Renderer2D::rotateAround(float inX, float inY, float &outX, float &outY,
							  float sin, float cos) {
