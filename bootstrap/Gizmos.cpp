...
Gizmos::Gizmos(unsigned int maxLines, unsigned int maxTris,
	...
	: m_maxLines(maxLines), m_lineCount(0), m_lines(new GizmoLine[maxLines]),
	  m_maxTris(maxTris), m_triCount(0), m_tris(new GizmoTri[maxTris]),
	  m_transparentTriCount(0), m_transparentTris(new GizmoTri[maxTris]),
	  m_max2DLines(max2DLines), m_2DlineCount(0),
	  m_2Dlines(new GizmoLine[max2DLines]), m_max2DTris(max2DTris),
	  m_2DtriCount(0), m_2Dtris(new GizmoTri[max2DTris]) {
  ...
  glBufferData(GL_ARRAY_BUFFER, m_maxLines * sizeof(GizmoLine), m_lines,
			   GL_DYNAMIC_DRAW);

  glGenBuffers(1, &m_triVBO);
  glBindBuffer(GL_ARRAY_BUFFER, m_triVBO);
  glBufferData(GL_ARRAY_BUFFER, m_maxTris * sizeof(GizmoTri), m_tris,
			   GL_DYNAMIC_DRAW);

  glGenBuffers(1, &m_transparentTriVBO);
  glBindBuffer(GL_ARRAY_BUFFER, m_transparentTriVBO);
  glBufferData(GL_ARRAY_BUFFER, m_maxTris * sizeof(GizmoTri), m_transparentTris,
			   GL_DYNAMIC_DRAW);

  glGenBuffers(1, &m_2DlineVBO);
  glBindBuffer(GL_ARRAY_BUFFER, m_2DlineVBO);
  glBufferData(GL_ARRAY_BUFFER, m_max2DLines * sizeof(GizmoLine), m_2Dlines,
			   GL_DYNAMIC_DRAW);

  glGenBuffers(1, &m_2DtriVBO);
  glBindBuffer(GL_ARRAY_BUFFER, m_2DtriVBO);
  glBufferData(GL_ARRAY_BUFFER, m_max2DTris * sizeof(GizmoTri), m_2Dtris,
			   GL_DYNAMIC_DRAW);
...
// Adds 3 unit-length lines (red,green,blue) representing the 3 axis of a
// transform, at the transform's translation. Optional scale available.
...
void Gizmos::addAABB(const glm::vec3 &center, const glm::vec3 &rvExtents,
...
void Gizmos::addAABBFilled(const glm::vec3 &center, const glm::vec3 &rvExtents,
...
void Gizmos::addCylinderFilled(const glm::vec3 &center, float radius,
  ...
  glm::vec3 tempCenter =
	  transform != nullptr ? glm::vec3((*transform)[3]) + center : center;

  float segmentSize = (2 * glm::pi<float>()) / segments;

  for (unsigned int i = 0; i < segments; ++i) {
	glm::vec3 v0top(0, fHalfLength, 0);
	glm::vec3 v1top(sinf(i * segmentSize) * radius, fHalfLength,
					cosf(i * segmentSize) * radius);
	glm::vec3 v2top(sinf((i + 1) * segmentSize) * radius, fHalfLength,
					cosf((i + 1) * segmentSize) * radius);
	glm::vec3 v0bottom(0, -fHalfLength, 0);
	glm::vec3 v1bottom(sinf(i * segmentSize) * radius, -fHalfLength,
					   cosf(i * segmentSize) * radius);
	glm::vec3 v2bottom(sinf((i + 1) * segmentSize) * radius, -fHalfLength,
					   cosf((i + 1) * segmentSize) * radius);
	...
	// triangles
	addTri(tempCenter + v0top, tempCenter + v1top, tempCenter + v2top,
		   fillColour);
	addTri(tempCenter + v0bottom, tempCenter + v2bottom, tempCenter + v1bottom,
		   fillColour);
	addTri(tempCenter + v2top, tempCenter + v1top, tempCenter + v1bottom,
		   fillColour);
	addTri(tempCenter + v1bottom, tempCenter + v2bottom, tempCenter + v2top,
		   fillColour);
...
void Gizmos::addRing(const glm::vec3 &center, float innerRadius,
					 float outerRadius, unsigned int segments,
					 const glm::vec4 &fillColour, const glm::mat4 *transform) {
  ...
  for (unsigned int i = 0; i < segments; ++i) {
	glm::vec3 v1outer(sinf(i * fSegmentSize) * outerRadius, 0,
					  cosf(i * fSegmentSize) * outerRadius);
	glm::vec3 v2outer(sinf((i + 1) * fSegmentSize) * outerRadius, 0,
					  cosf((i + 1) * fSegmentSize) * outerRadius);
	glm::vec3 v1inner(sinf(i * fSegmentSize) * innerRadius, 0,
					  cosf(i * fSegmentSize) * innerRadius);
	glm::vec3 v2inner(sinf((i + 1) * fSegmentSize) * innerRadius, 0,
					  cosf((i + 1) * fSegmentSize) * innerRadius);
	...
	if (fillColour.w != 0) {
	  addTri(tempCenter + v2outer, tempCenter + v1outer, tempCenter + v1inner,
			 fillColour);
	  addTri(tempCenter + v1inner, tempCenter + v2inner, tempCenter + v2outer,
			 fillColour);

	  addTri(tempCenter + v1inner, tempCenter + v1outer, tempCenter + v2outer,
			 fillColour);
	  addTri(tempCenter + v2outer, tempCenter + v2inner, tempCenter + v1inner,
			 fillColour);
	} else {
	  // line
...
void Gizmos::addDisk(const glm::vec3 &center, float radius,
					 unsigned int segments, const glm::vec4 &fillColour,
					 const glm::mat4 *transform) {
  ...
  for (unsigned int i = 0; i < segments; ++i) {
	glm::vec3 v1outer(sinf(i * fSegmentSize) * radius, 0,
					  cosf(i * fSegmentSize) * radius);
	glm::vec3 v2outer(sinf((i + 1) * fSegmentSize) * radius, 0,
					  cosf((i + 1) * fSegmentSize) * radius);
	...
	if (fillColour.w != 0) {
	  addTri(tempCenter, tempCenter + v1outer, tempCenter + v2outer,
			 fillColour);
	  addTri(tempCenter + v2outer, tempCenter + v1outer, tempCenter,
			 fillColour);
	} else {
	  // line
	  addLine(tempCenter + v1outer, tempCenter + v2outer, vSolid, vSolid);
...
void Gizmos::addArc(const glm::vec3 &center, float rotation, float radius,
					float arcHalfAngle, unsigned int segments,
					const glm::vec4 &fillColour, const glm::mat4 *transform) {
  ...
  for (unsigned int i = 0; i < segments; ++i) {
	glm::vec3 v1outer(
		sinf(i * fSegmentSize - arcHalfAngle + rotation) * radius, 0,
		cosf(i * fSegmentSize - arcHalfAngle + rotation) * radius);
	glm::vec3 v2outer(
		sinf((i + 1) * fSegmentSize - arcHalfAngle + rotation) * radius, 0,
		cosf((i + 1) * fSegmentSize - arcHalfAngle + rotation) * radius);
  ...
  // edge lines
  if (fillColour.w == 0) {
	glm::vec3 v1outer(sinf(-arcHalfAngle + rotation) * radius, 0,
					  cosf(-arcHalfAngle + rotation) * radius);
	glm::vec3 v2outer(sinf(arcHalfAngle + rotation) * radius, 0,
					  cosf(arcHalfAngle + rotation) * radius);
...
void Gizmos::addArcRing(const glm::vec3 &center, float rotation,
  ...
  for (unsigned int i = 0; i < segments; ++i) {
	glm::vec3 v1outer(
		sinf(i * fSegmentSize - arcHalfAngle + rotation) * outerRadius, 0,
		cosf(i * fSegmentSize - arcHalfAngle + rotation) * outerRadius);
	glm::vec3 v2outer(
		sinf((i + 1) * fSegmentSize - arcHalfAngle + rotation) * outerRadius, 0,
		cosf((i + 1) * fSegmentSize - arcHalfAngle + rotation) * outerRadius);
	glm::vec3 v1inner(
		sinf(i * fSegmentSize - arcHalfAngle + rotation) * innerRadius, 0,
		cosf(i * fSegmentSize - arcHalfAngle + rotation) * innerRadius);
	glm::vec3 v2inner(
		sinf((i + 1) * fSegmentSize - arcHalfAngle + rotation) * innerRadius, 0,
		cosf((i + 1) * fSegmentSize - arcHalfAngle + rotation) * innerRadius);
  ...
  // edge lines
  if (fillColour.w == 0) {
	glm::vec3 v1outer(sinf(-arcHalfAngle + rotation) * outerRadius, 0,
					  cosf(-arcHalfAngle + rotation) * outerRadius);
	glm::vec3 v2outer(sinf(arcHalfAngle + rotation) * outerRadius, 0,
					  cosf(arcHalfAngle + rotation) * outerRadius);
	glm::vec3 v1inner(sinf(-arcHalfAngle + rotation) * innerRadius, 0,
					  cosf(-arcHalfAngle + rotation) * innerRadius);
	glm::vec3 v2inner(sinf(arcHalfAngle + rotation) * innerRadius, 0,
					  cosf(arcHalfAngle + rotation) * innerRadius);
...
void Gizmos::addSphere(const glm::vec3 &center, float radius, int rows,
					   int columns, const glm::vec4 &fillColour,
  ...
  for (int row = 0; row <= rows; ++row) {
	// y ordinates this may be a little confusing but here we are navigating
	// around the xAxis in GL
	float ratioAroundXAxis = float(row) * invRows;
	float radiansAboutXAxis =
		ratioAroundXAxis * latitiudinalRange + (latMin * DEG2RAD);
	...
	for (int col = 0; col <= columns; ++col) {
	  ...
	  glm::vec3 v4Normal(inverseRadius * v4Point.x, inverseRadius * v4Point.y,
						 inverseRadius * v4Point.z);
  ...
  for (int face = 0; face < (rows) * (columns); ++face) {
	...
	addLine(tempCenter + v4Array[face], tempCenter + v4Array[face + columns],
			glm::vec4(1.f, 1.f, 1.f, 1.f), glm::vec4(1.f, 1.f, 1.f, 1.f));

	if (face % columns == 0 && longitudinalRange < (glm::pi<float>() * 2))
	  continue;
	addLine(tempCenter + v4Array[iNextFace + columns],
			tempCenter + v4Array[face + columns], glm::vec4(1.f, 1.f, 1.f, 1.f),
			glm::vec4(1.f, 1.f, 1.f, 1.f));

	addTri(tempCenter + v4Array[iNextFace + columns],
		   tempCenter + v4Array[face], tempCenter + v4Array[iNextFace],
		   fillColour);
	addTri(tempCenter + v4Array[iNextFace + columns],
		   tempCenter + v4Array[face + columns], tempCenter + v4Array[face],
		   fillColour);
  }
...
void Gizmos::addCapsule(const glm::vec3 &center, float height, float radius,
						int rows, int cols, const glm::vec4 &fillColour,
  ...
  for (int face = 0; face < (rows) * (cols); ++face) {
	...
	addLine(tempCenter + v4Array[face], tempCenter + v4Array[face + cols],
			white, white);

	if (face % cols == 0 && longitudinalRange < (glm::pi<float>() * 2))
	  continue;

	addLine(tempCenter + v4Array[iNextFace + cols],
			tempCenter + v4Array[face + cols], white, white);
	addTri(tempCenter + v4Array[iNextFace + cols], tempCenter + v4Array[face],
		   tempCenter + v4Array[iNextFace], fillColour);
	addTri(tempCenter + v4Array[iNextFace + cols],
		   tempCenter + v4Array[face + cols], tempCenter + v4Array[face],
		   fillColour);
  }
  ...
  for (int i = 0; i < cols; ++i) {
	...
	addTri(topCenter + pos1, bottomCenter + pos1, bottomCenter + pos,
		   fillColour);
...
void Gizmos::addHermiteSpline(const glm::vec3 &start, const glm::vec3 &end,
							  const glm::vec3 &tangentStart,
							  const glm::vec3 &tangentEnd,
							  unsigned int segments, const glm::vec4 &colour) {
  ...
  for (unsigned int i = 1; i <= segments; ++i) {
	...
	float h4 = s3 - s2;
	glm::vec3 p =
		(start * h1) + (end * h2) + (tangentStart * h3) + (tangentEnd * h4);
...
void Gizmos::addLine(const glm::vec3 &v0, const glm::vec3 &v1,
...
void Gizmos::addTri(const glm::vec3 &v0, const glm::vec3 &v1,
					const glm::vec3 &v2, const glm::vec4 &colour) {
  if (sm_singleton != nullptr) {
	if (colour.w == 1) {
	  ...
	  if (sm_singleton->m_transparentTriCount < sm_singleton->m_maxTris) {
		sm_singleton->m_transparentTris[sm_singleton->m_transparentTriCount]
			.v0.x = v0.x;
		sm_singleton->m_transparentTris[sm_singleton->m_transparentTriCount]
			.v0.y = v0.y;
		sm_singleton->m_transparentTris[sm_singleton->m_transparentTriCount]
...
void Gizmos::add2DAABB(const glm::vec2 &center, const glm::vec2 &extents,
					   const glm::vec4 &colour,
...
void Gizmos::add2DAABBFilled(const glm::vec2 &center, const glm::vec2 &extents,
							 const glm::vec4 &colour,
							 const glm::mat4 *transform /*= nullptr*/) {
  glm::vec2 verts[4];
  glm::vec2 vX(extents.x, 0);
...
void Gizmos::add2DCircle(const glm::vec2 &center, float radius,
						 unsigned int segments, const glm::vec4 &colour,
						 const glm::mat4 *transform /*= nullptr*/) {
  ...
  for (unsigned int i = 0; i < segments; ++i) {
	glm::vec2 v1outer(sinf(i * segmentSize) * radius,
					  cosf(i * segmentSize) * radius);
	glm::vec2 v2outer(sinf((i + 1) * segmentSize) * radius,
					  cosf((i + 1) * segmentSize) * radius);
...
void Gizmos::add2DLine(const glm::vec2 &rv0, const glm::vec2 &rv1,
					   const glm::vec4 &colour) {
...
void Gizmos::add2DTri(const glm::vec2 &rv0, const glm::vec2 &rv1,
					  const glm::vec2 &rv2, const glm::vec4 &colour) {
  add2DTri(rv0, rv1, rv2, colour, colour, colour);
}

void Gizmos::add2DTri(const glm::vec2 &rv0, const glm::vec2 &rv1,
					  const glm::vec2 &rv2, const glm::vec4 &colour0,
					  const glm::vec4 &colour1, const glm::vec4 &colour2) {
  if (sm_singleton != nullptr) {
...
void Gizmos::draw(const glm::mat4 &projectionView) {
  if (sm_singleton != nullptr &&
	...
	unsigned int projectionViewUniform =
		glGetUniformLocation(sm_singleton->m_shader, "ProjectionView");
	...
	if (sm_singleton->m_lineCount > 0) {
	  ...
	  glBufferSubData(GL_ARRAY_BUFFER, 0,
					  sm_singleton->m_lineCount * sizeof(GizmoLine),
					  sm_singleton->m_lines);
	...
	if (sm_singleton->m_triCount > 0) {
	  ...
	  glBufferSubData(GL_ARRAY_BUFFER, 0,
					  sm_singleton->m_triCount * sizeof(GizmoTri),
					  sm_singleton->m_tris);
	...
	if (sm_singleton->m_transparentTriCount > 0) {
	  ...
	  glBufferSubData(GL_ARRAY_BUFFER, 0,
					  sm_singleton->m_transparentTriCount * sizeof(GizmoTri),
					  sm_singleton->m_transparentTris);
...
void Gizmos::draw2D(const glm::mat4 &projection) {
  if (sm_singleton != nullptr &&
	  (sm_singleton->m_2DlineCount > 0 || sm_singleton->m_2DtriCount > 0)) {
	int shader = 0;
	glGetIntegerv(GL_CURRENT_PROGRAM, &shader);
	...
	if (sm_singleton->m_2DlineCount > 0) {
	  ...
	  glBufferSubData(GL_ARRAY_BUFFER, 0,
					  sm_singleton->m_2DlineCount * sizeof(GizmoLine),
					  sm_singleton->m_2Dlines);
	...
	if (sm_singleton->m_2DtriCount > 0) {
	  ...
	  glBufferSubData(GL_ARRAY_BUFFER, 0,
					  sm_singleton->m_2DtriCount * sizeof(GizmoTri),
					  sm_singleton->m_2Dtris);
