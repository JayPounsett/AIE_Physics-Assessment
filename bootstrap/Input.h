...
public:
  ...
  typedef std::function<void(GLFWwindow *window, int key, int scancode,
							 int action, int mods)>
	  KeyCallback;
  typedef std::function<void(GLFWwindow *window, unsigned int character)>
	  CharCallback;
  typedef std::function<void(GLFWwindow *window, int button, int action,
							 int mods)>
	  MouseButtonCallback;
  typedef std::function<void(GLFWwindow *window, double xoffset,
							 double yoffset)>
	  MouseScrollCallback;
  typedef std::function<void(GLFWwindow *window, double x, double y)>
	  MouseMoveCallback;
...
protected:
  ...
  // should be called once by the application each frame after the current
  // update or before glfwPollEvents
  void clearStatus();
...
private:
  ...
  bool m_firstMouseMove; // flag for first mouse input after start or mouse
						 // entering window
